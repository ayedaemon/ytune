"""
Shared SQL for the local_librosa table. ytm-sync seeds pending rows,
ytdlp-downloader fills in file_path once a track is downloaded, enrich-librosa
claims/completes them, api-gateway reads status and can force a re-analysis —
...
async def status_counts(conn: asyncpg.Connection) -> dict[str, int]:
    rows = await conn.fetch("SELECT status, COUNT(*) AS count FROM local_librosa GROUP BY status")
    return {r["status"]: r["count"] for r in rows}


async def claim_pending(conn: asyncpg.Connection, limit: int = 10) -> list[asyncpg.Record]:
    """
    Atomically claim up to `limit` pending rows that actually have a file to analyze —
    ...
    async with conn.transaction():
        rows = await conn.fetch(
            """
            SELECT video_id, file_path, retries FROM local_librosa
        ...
        if rows:
            await conn.execute(
                "UPDATE local_librosa SET status='processing', updated_at=NOW() WHERE video_id = ANY($1)",
                [r["video_id"] for r in rows],
            )
    return list(rows)


async def mark_done(conn: asyncpg.Connection, video_id: str, features: dict) -> None:
    await conn.execute(
        """
        UPDATE local_librosa SET status='done', features=$1, updated_at=NOW()
        WHERE video_id=$2
        """,
        json.dumps(features),
        video_id,
...
async def mark_failed_or_retry(
    ...
    if row["retries"] >= max_retries:
        await conn.execute(
            """
            UPDATE local_librosa SET status='error', error_message=$1, updated_at=NOW()
            WHERE video_id=$2
            """,
            error,
            video_id,
        ...
        await conn.execute(
            """
            UPDATE local_librosa SET status='pending', retries=retries+1, error_message=$1, updated_at=NOW()
            WHERE video_id=$2
            """,
            error,
            video_id,
...
async def reset_to_pending(conn: asyncpg.Connection, video_id: str) -> None:
    await conn.execute(
        """
        UPDATE local_librosa SET status='pending', retries=0, error_message=NULL, updated_at=NOW()
        WHERE video_id=$1
        """,
        video_id,
    )
...
async def notify(conn: asyncpg.Connection, video_id: str, event: str = "track_added") -> None:
    ...
    await conn.execute("SELECT pg_notify('track_librosa_queue', $1)", payload)
...
async def reset_many(conn: asyncpg.Connection, video_ids: list[str] | None = None, force: bool = False) -> dict:
    ...
    return await bulk.reset_many(
        conn, "local_librosa", "processing", "track_librosa_queue", video_ids, force,
