"""
Shared SQL for the local_mert table. ytm-sync seeds pending rows,
ytdlp-downloader fills in file_path, embeddings-mert claims/completes them,
api-gateway reads status and can force a re-embed.
"""
...
async def status_counts(conn: asyncpg.Connection) -> dict[str, int]:
    rows = await conn.fetch("SELECT status, COUNT(*) AS count FROM local_mert GROUP BY status")
...
async def claim_pending(conn: asyncpg.Connection, limit: int = 10) -> list[asyncpg.Record]:
    async with conn.transaction():
        rows = await conn.fetch(
            """
            SELECT video_id, file_path, retries FROM local_mert
        ...
        if rows:
            await conn.execute(
                "UPDATE local_mert SET status='processing', updated_at=NOW() WHERE video_id = ANY($1)",
...
async def mark_done(conn: asyncpg.Connection, video_id: str, embedding: list[float]) -> None:
    await conn.execute(
        """
        UPDATE local_mert SET status='done', embedding=$1, updated_at=NOW()
        WHERE video_id=$2
        """,
...
async def mark_failed_or_retry(
    ...
    if row["retries"] >= max_retries:
        await conn.execute(
            """
            UPDATE local_mert SET status='error', error_message=$1, updated_at=NOW()
            WHERE video_id=$2
            """,
            error,
            video_id,
        ...
        await conn.execute(
            """
            UPDATE local_mert SET status='pending', retries=retries+1, error_message=$1, updated_at=NOW()
            WHERE video_id=$2
            """,
            error,
            video_id,
...
async def reset_to_pending(conn: asyncpg.Connection, video_id: str) -> None:
    await conn.execute(
        """
        UPDATE local_mert SET status='pending', retries=0, error_message=NULL, updated_at=NOW()
        WHERE video_id=$1
        """,
        video_id,
    )
...
async def notify(conn: asyncpg.Connection, video_id: str, event: str = "track_added") -> None:
    ...
    await conn.execute("SELECT pg_notify('track_mert_queue', $1)", payload)
...
async def reset_many(conn: asyncpg.Connection, video_ids: list[str] | None = None, force: bool = False) -> dict:
    return await bulk.reset_many(
        conn, "local_mert", "processing", "track_mert_queue", video_ids, force,
