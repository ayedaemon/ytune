"""
CLAP embedding queue control. Thin wrappers over core.db.local_clap, plus one
...
from core.db.local_clap import reset_many, status_counts


async def _list_errors(conn: asyncpg.Connection, limit: int) -> list[dict]:
    rows = await conn.fetch(
        """
        SELECT lc.video_id, yt.title, yt.artist, lc.retries, lc.error_message, lc.updated_at
        FROM local_clap lc
        JOIN yt_tracks yt ON yt.id = lc.video_id
...
async def get_track_clap(conn: asyncpg.Connection, video_id: str) -> dict | None:
