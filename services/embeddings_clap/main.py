"""
embeddings-clap worker entrypoint.

Lifecycle:
  1. Connect pool.
  2. Recover any rows stuck in 'processing' from a previous crash.
  3. Open a dedicated LISTEN connection on 'track_clap_queue'.
...
from services.embeddings_clap.app.domain.embed import process_pending_clap
...
async def _recover_stuck(pool: asyncpg.Pool) -> None:
    ...
    async with pool.acquire() as conn:
        ...
        if affected:
            log.info("recovered_stuck_rows", table="local_clap", count=int(affected))
...
async def main() -> None:
    ...
    log.info("listening", channel="track_clap_queue")
    ...
    while True:
        ...
        await process_pending_clap(pool, settings)
