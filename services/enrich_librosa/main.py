"""
enrich-librosa worker entrypoint.

Lifecycle:
  1. Connect pool.
  2. Recover any rows stuck in 'processing' from a previous crash.
  3. Open a dedicated LISTEN connection on 'track_librosa_queue'.
  4. Drain any pending analyses queued before this process started.
...
from core.config import LibrosaSettings
from core.logging import configure_logging
from services.enrich_librosa.app.domain.analysis import process_pending_librosa

settings = LibrosaSettings()
...
async def _recover_stuck(pool: asyncpg.Pool) -> None:
    """Reset any rows left in 'processing' from a prior crash."""
    async with pool.acquire() as conn:
        affected = await conn.fetchval(
            f"""
            WITH updated AS (
                UPDATE local_librosa
        ...
        if affected:
            log.info("recovered_stuck_rows", table="local_librosa", count=int(affected))
...
async def main() -> None:
    ...
    log.info("listening", channel="track_librosa_queue")
    ...
    while True:
        ...
        await process_pending_librosa(pool, settings)
